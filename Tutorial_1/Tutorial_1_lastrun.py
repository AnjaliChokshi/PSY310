...
expName = 'Tutorial_1'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\hp\\Documents\\Anjali Assignments\\Lab in Psych\\Tutorial1\\Tutorial_1_lastrun.py',
...
conditions = data.importConditions('loop_settings.xlsx')
staircase_loop = data.MultiStairHandler(stairType='simple', name='staircase_loop',
    nTrials=100.0,
...
level = staircase_loop._nextIntensity  # initialise some vals
condition = staircase_loop.currentStaircase.condition

for level, condition in staircase_loop:
    ...
    staircase_loop.addOtherData('ISI.started', ISI.tStart)
    staircase_loop.addOtherData('ISI.stopped', ISI.tStop)
    staircase_loop.addOtherData('grating.started', grating.tStartRefresh)
    staircase_loop.addOtherData('grating.stopped', grating.tStopRefresh)
    # check responses
    ...
    # store data for staircase_loop (MultiStairHandler)
    staircase_loop.addResponse(key_resp.corr)
    staircase_loop.addOtherData('key_resp.rt', key_resp.rt)
    staircase_loop.addOtherData('key_resp.started', key_resp.tStartRefresh)
    staircase_loop.addOtherData('key_resp.stopped', key_resp.tStopRefresh)
